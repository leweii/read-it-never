...
- [Content Types](#content-types)
    ...
    - [Podcast (RSS feed)](#podcast-rss-feed)
...
Matches episode pages on [xiaoyuzhoufm.com](https://www.xiaoyuzhoufm.com) (小宇宙), e.g. `https://www.xiaoyuzhoufm.com/episode/<id>`.
...
| episodeURL                 | Episode URL on xiaoyuzhoufm.com                                |
| episodePlayer               | Embedded `<audio>` player generated by plugin                  |
| episodeDescription          | Episode shownotes, converted to Markdown                       |
| episodeThumbnail            | Episode cover image URL                                        |
| episodeDuration             | Episode duration in seconds                                    |
| episodeDurationFormatted    | Formatted episode duration (1h 25m 23s)                        |
| episodePublishDate          | Episode publish date formatted in content format from settings |
| podcastName                 | Podcast (show) name                                             |
| podcastAuthor               | Podcast author/host name                                        |
| podcastURL                  | Podcast page URL on xiaoyuzhoufm.com                            |
| podcastThumbnail            | Podcast cover image URL                                         |
| audioURL                    | Direct audio file URL                                           |

Metadata is scraped from the episode page's embedded `__NEXT_DATA__` JSON (no login or official API required).

### Podcast (RSS feed)

Matches URLs that look like a podcast RSS/Atom feed (ending in `.xml`/`.rss`, or containing a `/feed` or `/rss` path segment). The note is generated from the most recent `<item>` in the feed, so this works for podcast platforms other than Xiaoyuzhou as long as you paste the feed URL rather than an episode page.
...
| episodeTitle               | Episode title                                                   |
| date                        | Current date in format from plugin settings                     |
| episodeURL                  | Episode link from the feed item                                 |
| episodePlayer               | Embedded `<audio>` player generated by plugin                   |
| episodeDescription          | Episode description/shownotes, converted to Markdown            |
| episodeThumbnail            | Episode or podcast cover image URL                               |
| episodeDurationFormatted    | Formatted episode duration (1h 25m 23s), if the feed provides one |
| episodePublishDate          | Episode publish date formatted in content format from settings   |
| podcastName                 | Podcast (channel) name                                           |
| podcastAuthor               | Podcast author, from `itunes:author`                             |
| feedURL                     | The feed URL that was fetched                                    |
| audioURL                    | Direct audio file URL from the item's `<enclosure>`              |
